...
def build_model(tparams, options, Wemb):
    ...
    #emb = Wemb[x.flatten()].reshape([n_timesteps, n_samples, n_value])

    proj = gru_layer(tparams, x, options, mask=mask)
...
def padMatrix(seqs):
    ...
    x = np.zeros((maxlen, n_samples, 9)).astype('int32')
...
if __name__ == '__main__':
    ...
    labelFile = 'label.pkl'
    outFile = 'out.txt'
    inputDimSize = 9  # The number of unique medical codes
    ...
    train_GRU_RNN(dataFile=dataFile, labelFile=labelFile, outFile=outFile, inputDimSize=inputDimSize,
                  hiddenDimSize=hiddenDimSize, max_epochs=max_epochs, L2_reg=L2_reg, batchSize=batchSize,
